...
<div class="w-4/5 m-auto pt-20">
    <form
        ...
        <div class="mb-6">
            ...
            <input type="text" name="title" value="{{$post->title}}" id="large-input" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        </div>
        <div class="mb-6">
            <label for="large-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Content</label>
            <textarea name="description" id="message" value="{{$post->description}}" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Content...">{{$post->description}}</textarea>
        </div>

        <div class="mb-6">
            <label for="large-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tag:</label>
            <input type="text" name="tags"  value="@foreach($post->tag as $tag){{$tag->tag}}, @endforeach" id="tagAdded" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" readonly>
        </div>
...
<script>
    ...
    let tags = document.getElementById('tagAdded').value;
    tags = tags.split(', ');
    tags.forEach(tag => {
        tagArray.push(tag);
    });
    ...
    for(let i = 0; i < tagArray.length; i++) {
        if(document.getElementById('tagAdded').value === '')
            document.getElementById('tagAdded').value = tagArray[i];
        else if(document.getElementById('tagAdded').value !== '')
        document.getElementById('tagAdded').value += ', ' + tagArray[i];
    }
